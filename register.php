 ...
 if(!empty($_POST)){
	if(isset($_POST['name'],$_POST['dob'],$_POST['sex'],$_POST['ms'],$_POST['pn'],$_POST['bio'],$_POST['un'],$_POST['pass'])){
		...
		if(!empty($name) && !empty($dob) && !empty($sex) && !empty($ms) && !empty($pn) && !empty($bio) && !empty($username) && !empty($password)){
			...
			if($insert->execute()){
				echo "Registration successful! Login to Enter";
				?><input type="button" name="Log" value="LOGIN" onclick="window.location.href='login.php'" ><?php 
                   
		
	}
}
}else { echo "Please Enter correct values in all the fields!";}
} 
